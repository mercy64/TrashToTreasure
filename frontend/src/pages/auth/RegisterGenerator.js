      ...
      <div className="max-w-md w-full">
        <div className="bg-gradient-to-r from-blue-700 to-sky-600 text-white p-8 rounded-lg shadow-md">
          <h2 className="flex items-center justify-center gap-2 text-center text-2xl md:text-3xl font-extrabold text-white mb-2">
            ...
            <span>Create Waste Generator account</span>
          </h2>
          ...
          <form className="space-y-6" onSubmit={handleSubmit} aria-describedby={error ? 'register-error' : undefined}>
            {(fieldErrors.non_field_errors || error) && (
              <div id="register-error" role="alert" className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-md">
                {fieldErrors.non_field_errors ? (
                  Array.isArray(fieldErrors.non_field_errors) ? fieldErrors.non_field_errors.join(' ') : fieldErrors.non_field_errors
                ) : (
            ...
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="typical_waste_types" className="block text-sm font-medium text-white/90">Typical Waste Types</label>
                <p className="text-xs text-white/80 mb-1">This helps recyclers know what materials you commonly have.</p>
                <input id="typical_waste_types" name="typical_waste_types" value={formData.typical_waste_types} onChange={handleChange} className="input-field bg-white/90 text-sky-900 w-full rounded-md" placeholder="e.g., plastic, paper" />
                {fieldErrors.typical_waste_types && (
                  <p className="text-red-200 text-sm mt-1">{Array.isArray(fieldErrors.typical_waste_types) ? fieldErrors.typical_waste_types[0] : fieldErrors.typical_waste_types}</p>
                )}
              </div>
